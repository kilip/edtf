...
class Qualification
{
    ...
    public function uncertain(?string $part = null): bool
    {
        if(!is_null($part)){
            $this->validatePartName($part);
            return self::UNCERTAIN === $this->$part;
        }
    ...
    public function approximate(?string $part = null): bool
    {
        if(!is_null($part)){
            $this->validatePartName($part);
            return self::APPROXIMATE === $this->$part;
        }
    ...
    public function uncertainAndApproximate(?string $part = null): bool
    {
        if(!is_null($part)){
            $this->validatePartName($part);
            return self::UNCERTAIN_AND_APPROXIMATE === $this->$part;
        }
        return $this->uncertainAndApproximate('year') || $this->uncertainAndApproximate('month') || $this->uncertainAndApproximate('day');
    }
